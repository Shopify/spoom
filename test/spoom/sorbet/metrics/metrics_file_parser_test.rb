...
module Spoom
  module Sorbet
    module Metrics
      class MetricsFileParserTest < Minitest::Test
        def test_parses_metrics_error
          assert_raises(JSON::ParserError) do
            Spoom::Sorbet::Metrics::MetricsFileParser.parse_string("")
          end
        end

        def test_parses_metrics
          metrics = Spoom::Sorbet::Metrics::MetricsFileParser.parse_string(<<~ERR)
            {
            "repo": "MyRepo",
            "sha": "1234",
            "status": "Success",
        ...
        def test_parses_metrics_and_removes_prefix
          metrics = Spoom::Sorbet::Metrics::MetricsFileParser.parse_string(<<~ERR, "metrics.")
            {
            "repo": "MyRepo",
            "sha": "1234",
            "status": "Success",
