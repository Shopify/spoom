...
module Spoom
  class Model
    ...
    class SymbolNotFound < Error; end
    ...
    class Class < Scope
      ...
      def children_of?(full_name)
        superclass = self.superclass
        return true if superclass&.full_name == full_name
    ...
    def resolve_includes(scope)
      unresolved = scope.includes.dup
      scope.includes.clear

      unresolved.each do |inc|
        next unless inc.is_a?(Ref)

        included = resolve_name(inc.full_name, scope)

        if included.is_a?(Module)
          scope.includes << included
        else
          scope.includes << inc
        end
      end
    ...
    sig { params(full_name: String).returns(T::Array[T.any(Ref, Scope)]) }
    def parents_of(full_name)
      ancestors = T.let([], T::Array[T.any(Ref, Scope)])

      scopes = @scopes[full_name]
      return ancestors unless scopes&.any?

      scopes.each do |scope|
        if scope.is_a?(Class)
          superclass = scope.superclass
          ancestors << superclass if superclass
        end
    ...
    def ancestors_of(full_name)
      ...
      scopes.each do |scope|
        if scope.is_a?(Class)
          superclass = scope.superclass
          ancestors << superclass if superclass
          ancestors.concat(ancestors_of(superclass.full_name)) if superclass.is_a?(Class)
        end

        scope.includes.each do |inc|
          ancestors << inc
          ancestors.concat(ancestors_of(inc.full_name))
        end
      end
    ...
    sig { params(full_name: String).returns(T::Array[Scope]) }
    def children_of(full_name)
      # target = @scopes[full_name]
      # raise SymbolNotFound, "Can't find `#{full_name}` in the index" unless target&.any?

      descendants = T.let([], T::Array[Scope])

      scopes.each do |_full_name, scopes|
        scopes.each do |scope|
          descendants << scope if scope.children_of?(full_name)
        end
      end
    ...
    def descendants_of(full_name)
      ...
      scopes.each do |_full_name, scopes|
        scopes.each do |scope|
          descendants << scope if scope.descendant_of?(full_name)
        end
      end
