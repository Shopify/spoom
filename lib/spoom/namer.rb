...
module Spoom
  ...
  class Namer < Model::NamespaceVisitor
    extend T::Sig
    ...
    def initialize(model, file)
      ...
      @model = model
      @file = file
      @symbol_for_node = T.let({}, T::Hash[Prism::Node, Model::Symbol])
    end
    ...
    def visit_class_node(node)
      name = @names_nesting.join("::")
      symbol = @symbol_for_node[node.constant_path] = @model.symbols[name] || Model::UnresolvedSymbol.new(name)

      # symbol_def = symbol.definitions.select { |dfn| dfn.location == node.location }
    end

    sig { override.params(node: Prism::ClassNode).void }
    def visit_module_node(node)
      name = @names_nesting.join("::")
      symbol = @model.symbols[name]
      @symbol_for_node[node.constant_path] = symbol || Model::UnresolvedSymbol.new(name)
    end
