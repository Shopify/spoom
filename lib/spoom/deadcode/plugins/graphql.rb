      ...
      class GraphQL < Base
        ...
        def on_field(send)
          ...
          send.each_arg_assoc do |key, value|
            ...
            next unless FIELD_SYMBOL_OPTION_KEYS.include?(key.value)
            next unless value.is_a?(Prism::SymbolNode)

            @index.reference_method(value.value, send.location)
          end
        end
        ...
        def on_argument(send)
          send.each_arg_assoc do |key, value|
            next unless key.is_a?(Prism::SymbolNode)
            next unless ARGUMENT_SYMBOL_OPTION_KEYS.include?(key.value)
            next unless value.is_a?(Prism::SymbolNode)
